...
def unpack_request(request: Any, expected_num_attributes: int) -> List[List[Any]]:
    ...
    if "data" not in request.keys():
        raise AttributeError(f"Request body does not match the expected format. (no data) Recieved: {json.dumps(request)}")
    if not isinstance(request["data"], list):
        raise AttributeError(f"Request body does not match the expected format. (not a list) Recieved: {json.dumps(request)}")
    if len(request["data"]) < 1:
        raise AttributeError(f"Request body does not match the expected format. (nothing in data) Recieved: {json.dumps(request)}")
